...
def test_get_l1_requirements_success():
    ...
    mock_response_data = {
        ...
        "issues": [
            {"id": 1, "fields": {"name": "L1 Jira Requirement 1"}},
            {"id": 2, "fields": {"name": "L1 Jira Requirement 2"}},
        ],
    }
    ...
    with patch("ska_jama_jira_integration.jira.api_interface.requests.get") as mock_get:
        ...
        # Test Jira get_l1_requirements
        result = get_l1_requirements()

        assert isinstance(result, list)
        assert len(result) == 2
        assert result[0]["id"] == 1
        assert result[1]["fields"]["name"] == "L1 Jira Requirement 2"
